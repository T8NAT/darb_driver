...
registerDriver() async {
  print('identityNumber => $identityNumber');
  print('sequenceNumber => $sequenceNumber');
  print('dateOfBirthGregorian => $dateOfBirthGregorian');
  print('dateOfBirthHijri => $dateOfBirthHijri');
  ...
  try {
    ...
    response.fields.addAll({
      ...
      "dateOfBirthGregorian": dateOfBirthGregorian.toString(),
      //.replaceAll('/', '-'),
...
/*  if ((identityNumber?.isEmpty ?? true) ||
      (sequenceNumber?.isEmpty ?? true) ||
      (dateOfBirthGregorian?.isEmpty ?? true) ||
      (dateOfBirthHijri?.isEmpty ?? true) ||
      (carNumber?.isEmpty ?? true)) {
    return 'All fields must be filled';
  }*/

  try {
    // Construct the request body with all mandatory and optional fields
    Map<String, String> body = {
      "role": ischeckownerordriver,
    };

    // Add optional fields only if they are provided
    if (identityNumber != null) body["identityNumber"] = identityNumber;
    if (sequenceNumber != null) body["sequenceNumber"] = sequenceNumber;
    if (dateOfBirthGregorian != null)
      body["dateOfBirthGregorian"] = dateOfBirthGregorian;
    if (dateOfBirthHijri != null) body["dateOfBirthHijri"] = dateOfBirthHijri;
    if (carNumber != null) body["carNumber"] = carNumber;

    // Add mobile or email depending on the conditions
    if (number.isNotEmpty && email.isNotEmpty) {
      body["mobile"] = number;
      body["email"] = email;
    } else if (login == 0) {
      body["mobile"] = number;
...
getUserDetails() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      currentRadius =
          (double.tryParse(userDetails['range_rides'].toString()) ?? 0);
...
updateMapRadius(radius) async {
  ...
  try {
    ...
    response.fields['range_rides'] = radius.toString();

  final res = await response.send();

  print('resres ${res.statusCode}');
  } catch (e) {
    print('eeeeeeeee $e');
    if (e is SocketException) {
      result = languages[choosenLanguage]['no internet'];
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (test == false) {
        ...
        result = languages[choosenLanguage]['something went wrong'];
      }
    }
    return result;
  } catch (e) {
...
sendOTPtoMobile(String mobile, String countryCode) async {
  ...
  try {
    ...
    // Validate mobile and countryCode
    if (mobile.isEmpty || countryCode.isEmpty) {
      return 'Mobile number and country code are required';
    }
    ...
    if (response.statusCode == 200) {
      ...
      if (responseBody['success'] == true) {
        ...
        debugPrint(response.body);
        result = responseBody['message'] ?? 'Something went wrong';
      }
    } else if (response.statusCode == 422) {
      debugPrint(response.body);
      var error = jsonDecode(response.body)['errors'];
      ...
      result = 'Failed to send OTP. Status code: ${response.statusCode}';
    }
    return result;
  } catch (e) {
    print('errrr $e');
...
validateSmsOtp(String mobile, String otp) async {
  ...
  try {
    ...
    if (mobile.isEmpty || otp.isEmpty) {
      return 'Mobile number and OTP are required';
    }
    ...
    if (response.statusCode == 200) {
      ...
      result = 'Failed to validate OTP. Status code: ${response.statusCode}';
    }
  } catch (e) {
    print('errrr $e');
    if (e is SocketException) {
